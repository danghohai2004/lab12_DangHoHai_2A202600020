...
```bash
# 1. Health check
curl https://captivating-alignment-production.up.railway.app/health
...
curl https://captivating-alignment-production.up.railway.app/ask
# Should return 401
...
Invoke-RestMethod -Uri "https://captivating-alignment-production.up.railway.app/ask" `
  ...
  -Headers @{
    "Content-Type" = "application/json"
    "X-API-Key" = "_XRQfXnUx54_3MHdGbHY52ji53....."
  } `
...
for ($i=1; $i -le 12; $i++) {
    try {
        $r = Invoke-RestMethod -Uri "https://captivating-alignment-production.up.railway.app/ask" `
          -Method Post `
          -Headers @{"X-API-Key" = "_XRQfXnUx54_3MHdGbHY52ji53uecOO5N........."; "Content-Type" = "application/json"} `
          -Body '{"question": "test"}'
        Write-Host "Req ${i}: Success" -ForegroundColor Green
    } catch {
        Write-Host "Req ${i}: Blocked (429)" -ForegroundColor Red
    }
}
done
